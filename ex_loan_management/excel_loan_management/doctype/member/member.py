...
def import_loan_members(file_url):
    ...
    try:
        ...
        for row in rows:
            ...
            # 4️⃣ If Member already exists → check & update group
            ...
            if existing_member:
				# Get Loan Group name from group_id
                new_group = frappe.db.get_value(
					"Loan Group",
					{"group_id": group_id},
					"name"
				) if group_id else None

				# Update group only if different
                if new_group and existing_member.group != new_group:
                    frappe.db.set_value(
						"Member",
                    ...
                    created_groups.append(f"{member_id} → {group_id}")
                else:
        ...
        # Prepare Summary
        msg = f"✅ Created {len(created_members)} MeLoanmbers."
        if created_groups:
...
def check_unique_member(member_name, pancard, current_name=None):
    """Check if Member with same name & PAN exists (excluding current)."""
    ...
    if existing:
        return "member with the same name and PAN already exists."
    return None
...
def create_loan_member():
    try:
        ...
        # Step 1: Create Member doc (without files)
        doc = frappe.get_doc({
            "doctype": "Member",
        ...
        frappe.log_error(frappe.get_traceback(), "Member Update API Error")
        return api_error(e)
...
def update_status(docname, status):
    """Update verification_status of a Member"""
...
def update_loan_member(name):
    try:
        ...
        return {
            "status": "success",
            "status_code": 201,
            "msg": "Member Sent for Verification Successfully"
        }
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Member Update API Error")
        return api_error(e)
...
def loan_member_get(name):
    """
    Get Member by name (primary key)
