...
from lending.loan_management.doctype.loan_repayment.loan_repayment import LoanRepayment as CoreLoanRepayment
from frappe.utils import nowdate


class CustomLoanRepayment(CoreLoanRepayment):
	def validate(self):
		print("in override ....................")
	...
	# Code added to set created_by as employee. 
	# This is required to make sure that collection in hand entry is 
	...
	def on_submit(self):
		print("in overide on submit ....")
		if self.repayment_type == "Normal Repayment" or self.repayment_type == "Loan Closure":
		...
		print("before  .....update_paid_amount .....")
	...
	def update_paid_amount(self):
		print("in check ............update_paid_amount .....")
		...
		print("loan.total_principal_paid ...",loan.total_principal_paid, '...self.principal_amount_paid..',self.principal_amount_paid, "self.self.payable_amount ...",self.payable_amount)
		print("loan.total_amount_paid,",loan.total_amount_paid ,"self.amount_paid,...",self.amount_paid)

        # This code is addded to handle the case when principal amount paid is more than payable principal amount due to 
		# shortfall recovery or excess payment from borrower side. 
		# In such cases, we will consider only payable principal amount as paid and excess amount will be adjusted in next repayment entry
		# Added by Rutuja Date: 3-APR-2026
		if self.repayment_type == "Loan Closure":
			principal_amount_paid = self.payable_amount if self.principal_amount_paid > self.payable_amount else self.principal_amount_paid
		else:
			principal_amount_paid = self.principal_amount_paid
		loan.update(
			{
				"total_amount_paid": loan.total_amount_paid + self.amount_paid,
				"total_principal_paid": loan.total_principal_paid + principal_amount_paid,
			}
		)

		pending_principal_amount = get_pending_principal_amount(loan)
		if (not loan.is_secured_loan and pending_principal_amount <= 0) or self.repayment_type == "Loan Closure":
			print("in if ........")
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		account_details = frappe.db.get_value(
			...
			[
				...
				"custom_closure_difference_account", # This is custom code added by rutuja. Date: 3-APR-2026
			],
			as_dict=1,
		)

        # This is custom code added by rutuja. Date: 3-APR-2026
        # Get difference account (for handling excess/shortfall)
		difference_account = getattr(self, "difference_account", None)
		if not difference_account:
			# Fallback to interest income account from Loan Product
			difference_account = account_details.custom_closure_difference_account
			if not difference_account:
				frappe.throw(
                    _("Please set Difference Account in Loan Repayment or ensure Loan Product has Interest Income Account")
                )
		...
		for repayment in self.get("repayment_details"):
			difference_amount = flt(self.amount_paid, precision) - (flt(self.pending_principal_amount, precision) + flt(self.interest_payable, precision) + flt(self.penalty_amount, precision))
			...
			if abs(difference_amount) > 0.001:
				if difference_amount > 0:
					print("self.loan_account. ...",self.loan_account, 'payment_account ',payment_account, 'difference_account ...',difference_account)
                    # Excess payment - income for the company
					gle_map.append(
                        self.get_gl_dict(
                            {
                                "account": payment_account,
                                "against": difference_account,
                                "debit": flt(difference_amount, precision),
                                "debit_in_account_currency": flt(difference_amount, precision),
                                "against_voucher_type": "Loan",
                                "against_voucher": self.against_loan,
                                "remarks": _("Excess amount in loan closure: ") + _(remarks),
                                "cost_center": self.cost_center,
                                "posting_date": getdate(self.posting_date),
                            },
                            item=self,
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "against": payment_account,
                                "credit": flt(difference_amount, precision),
                                "credit_in_account_currency": flt(difference_amount, precision),
                                "against_voucher_type": "Loan",
                                "against_voucher": self.against_loan,
                                "remarks": _("Excess amount in loan closure: ") + _(remarks),
                                "cost_center": self.cost_center,
                    ...
                    # Shortfall - expense/loss for the company
					shortfall = abs(difference_amount)
					gle_map.append(
                        self.get_gl_dict(
                            {
                                "account": difference_account,
                                "against": self.loan_account,
                                "debit": flt(shortfall, precision),
                                "debit_in_account_currency": flt(shortfall, precision),
                                "against_voucher_type": "Loan",
                                "against_voucher": self.against_loan,
                                "remarks": _("Shortfall in loan closure: ") + _(remarks),
                                "cost_center": self.cost_center,
                                "posting_date": getdate(self.posting_date),
                            },
                            item=self,
                        ...
                        self.get_gl_dict(
                            {
                                "account": self.loan_account,
                                "against": difference_account,
                                "credit": flt(shortfall, precision),
                                "credit_in_account_currency": flt(shortfall, precision),
                                "against_voucher_type": "Loan",
                                "against_voucher": self.against_loan,
                                "remarks": _("Shortfall in loan closure: ") + _(remarks),
                                "cost_center": self.cost_center,
		...
		if not self.get("repayment_details") and flt(self.pending_principal_amount, precision) > 0:
			...
			if flt(self.amount_paid, precision) > flt(self.pending_principal_amount, precision) and self.repayment_type != "Loan Closure":
				frappe.throw(
...
def get_amounts(amounts, against_loan, posting_date, with_loan_details=False):
	...
	for entry in accrued_interest_entries:
		...
		print("posting_date ...",posting_date,'...', due_date_after_grace_period,'....')
